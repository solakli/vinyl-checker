...
var ARCHETYPE_RULES = [
    // ── Electronic / Club ──────────────────────────────────────────────────────
    { label:'Detroit Techno Purist', color:'smoke',  styles:['Detroit Techno','Deep Techno'],                              genres:[],              min:5  },
    { label:'UK Garage Head',        color:'teal',   styles:['UK Garage','Speed Garage','2-Step'],                         genres:[],              min:5  },
    { label:'DnB / Jungle Head',     color:'purple', styles:['Drum n Bass','Jungle','Darkstep','Neurofunk'],               genres:[],              min:7  },
    { label:'Minimal / Rominimal',   color:'blue',   styles:['Minimal','Minimal Techno','Microhouse'],                     genres:[],              min:7  },
    { label:'Acid House Head',       color:'green',  styles:['Acid','Acid House','Acid Jazz','Acid Techno'],               genres:[],              min:5  },
    { label:'House Lifer',           color:'orange', styles:['Chicago House','Deep House','Soulful House','House'],        genres:[],              min:9  },
    { label:'Tech House Head',       color:'teal',   styles:['Tech House'],                                                genres:[],              min:5  },
    { label:'Electro Purist',        color:'yellow', styles:['Electro'],                                                   genres:[],              min:5  },
    { label:'Grime Head',            color:'red',    styles:['Grime'],                                                     genres:[],              min:5  },
    { label:'Italo / Cosmic Head',   color:'pink',   styles:['Cosmic','Italo-Disco','Space'],                              genres:[],              min:5  },
    { label:'Breaks Head',           color:'red',    styles:['Breakbeat','Breaks','Nu-Skool Breaks','Big Beat'],           genres:[],              min:5  },
    { label:'Dub Archaeologist',     color:'green',  styles:['Dub','Roots Reggae','Dub Techno','Lovers Rock'],             genres:['Reggae'],      min:7  },
    { label:'Ambient Architect',     color:'blue',   styles:['Ambient','Drone','New Age','Dark Ambient'],                  genres:[],              min:7  },
    { label:'Industrial / EBM',      color:'smoke',  styles:['EBM','Industrial','Dark Electro','Power Electronics'],       genres:[],              min:5  },
    { label:'Trance Head',           color:'purple', styles:['Trance','Progressive Trance','Psy-Trance'],                  genres:[],              min:7  },
    // ── Record Crate Genres ────────────────────────────────────────────────────
    { label:'Jazz Archaeologist',    color:'gold',   styles:['Bop','Post Bop','Hard Bop','Cool Jazz','Free Jazz'],         genres:['Jazz'],        min:8  },
    { label:'Soul & Funk Hunter',    color:'orange', styles:['Soul','Funk','Northern Soul','Neo Soul'],                    genres:['Soul'],        min:10 },
    { label:'Hip Hop Head',          color:'red',    styles:[],                                                            genres:['Hip Hop'],     min:12 },
    { label:'Synth / Post-Punk',     color:'pink',   styles:['Synth-pop','New Wave','Post-Punk','Darkwave'],               genres:[],              min:7  },
    { label:'World Grooves Head',    color:'gold',   styles:['Afrobeat','Highlife','Afro-Cuban','Cumbia','Baile Funk'],    genres:[],              min:4  },
    { label:'Latin Grooves Head',    color:'teal',   styles:['Cumbia','Salsa','Latin Jazz','Bossa Nova'],                  genres:['Latin'],       min:5  },
    { label:'Balearic Head',         color:'teal',   styles:['Balearic','Chill Out','Downtempo'],                          genres:[],              min:5  },
    { label:'Noise / Experimental',  color:'smoke',  styles:['Noise','Avant-garde','Free Improvisation'],                  genres:['Non-Music'],   min:5  },
    { label:'Classical Digger',      color:'gold',   styles:[],                                                            genres:['Classical'],   min:10 },
];
...
app.get('/api/diggers', function (req, res) {
    try {
        ...
        var result = users.map(function(u) {
            ...
            // Personality tags for badge overlap in match explanations
            var tagTotal = Object.values(profile.genres).reduce(function(s,n){return s+n;},0) +
                           Object.values(profile.styles).reduce(function(s,n){return s+n;},0);
            var pTags = computePersonalityTags(profile.genres, profile.styles, tagTotal, null, null);
            var row = {
                username:       u.username,
                wantlist:       wantlist,
                inStock:        inStock,
                ...
                topGenres:      topGenres,
                archetypeLabels: pTags.map(function(t) { return t.label; })
            };
...
function reapChrome() {
    // Uses async exec() — never blocks the event loop.
    // execSync() here used to freeze Node for 1-2s every 10 min, which made the
    // watchdog health check time out and restart a perfectly healthy server.
    ...
    if (!scannerBusy) {
        exec('pgrep -P ' + myPid + ' -f "chrom" 2>/dev/null || true', function(err, out) {
            if (err || !out || !out.trim()) return;
    ...
    exec("ps -eo pid,etimes,args --no-headers 2>/dev/null | awk '$2>14400 && /puppeteer_dev_profile/ {print $1}' || true", function(err, out) {
        if (err || !out || !out.trim()) return;
        var orphanPids = out.trim().split('\n').filter(Boolean);
        var killed = 0;
