...
async function runVideoIdSearch(apiKey, opts) {
    ...
    opts = opts || {};
    var batchSize = opts.batchSize || 90;   // 90 searches × 100 units = 9k quota (was 50)
    var delayMs   = opts.delayMs   || 3000; // 3s between searches → ~4.5 min per batch
    ...
    try {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            try {
                var videoId = await searchVideoId(item.artist, item.title, apiKey, { label: item.label });
                if (videoId) {
