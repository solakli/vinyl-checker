...
async function runYouTubeEnrichment(apiKeys, opts) {
    ...
    try {
        ...
        // This costs ~ceil(N/50) units instead of N units — huge savings.
        var statsCache = {};
        var STATS_BATCH = 50;
        var keyIdx = 0;
        for (var b = 0; b < items.length; b += STATS_BATCH) {
            ...
            try {
                ...
                console.log('[yt-enrich] Stats batch', Math.floor(b / STATS_BATCH) + 1,
                    '— fetched', Object.keys(batchStats).length, 'of', batch.length);
            } catch(e) {
                console.warn('[yt-enrich] Stats batch failed:', e.message);
            }
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (!s) {
                // Video deleted or API error during stats batch — skip
                continue;
            }
            ...
            try {
                ...
                // Comments disabled on this video — still save stats
            }
