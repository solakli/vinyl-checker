            ...
            res.on('end', function() {
                try {
                    ...
                    if (parsed.error) {
                        ...
                        // YouTube Data API returns code=403 with reason='quotaExceeded'.
                        // Google Cloud project-level quota returns code=429 or status='RESOURCE_EXHAUSTED'.
                        ...
                        }) || parsed.error.code === 403 || parsed.error.code === 429 ||
    ...
    try {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (!key) {
                ...
                upsert.run(s.viewCount, s.likeCount, s.commentCount, JSON.stringify({ genres: s.descGenres || [], era: [], djs: [], soundsLike: [], rawTop: [] }), s.publishedAt || null, item.discogs_id);
                done++;
                continue;
            }
            ...
            upsert.run(s.viewCount, s.likeCount, s.commentCount, JSON.stringify(commentData), s.publishedAt || null, item.discogs_id);
            done++;
