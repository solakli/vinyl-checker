...
// Scopes needed for listening history + library access
...
function spotifyAuthorizeUrl(state) {
    var callbackUrl = BASE_URL + '/api/auth/spotify/callback';
...
function _spotifyTokenRequest(postData) {
    return new Promise(function (resolve, reject) {
        var options = {
            ...
            method:   'POST',
            headers:  {
                'Authorization':  'Basic ' + _spotifyBasicAuth(),
                'Content-Type':   'application/x-www-form-urlencoded',
                'Content-Length': Buffer.byteLength(postData),
            },
        };
        var req = https.request(options, function (res) {
            ...
            res.on('end', function () {
                ...
                } catch (e) { reject(new Error('Spotify token parse error: ' + data.slice(0, 100))); }
            });
        });
        req.on('error', reject);
        req.write(postData);
...
function spotifyExchangeCode(code) {
    var callbackUrl = BASE_URL + '/api/auth/spotify/callback';
    return _spotifyTokenRequest(querystring.stringify({
        grant_type:   'authorization_code',
        code:         code,
        redirect_uri: callbackUrl,
    }));
...
function spotifyRefreshToken(refreshToken) {
    return _spotifyTokenRequest(querystring.stringify({
        grant_type:    'refresh_token',
        refresh_token: refreshToken,
        ...
        var req = https.request(options, function (res) {
            ...
            res.on('end', function () {
                try {
                    ...
                    var json = JSON.parse(data);
                    if (res.statusCode >= 400) {
                        var msg = (json.error && json.error.message) || 'Spotify API ' + res.statusCode;
                        return reject(new Error(msg));
                    }
                    resolve(json);
                } catch (e) { reject(new Error('Spotify parse error: ' + data.slice(0, 100))); }
            });
        });
        req.on('error', reject);
        if (body) req.write(postData);
...
function soundcloudEnabled() { return !!(SOUNDCLOUD_CLIENT_ID && SOUNDCLOUD_CLIENT_SECRET); }

function soundcloudAuthorizeUrl(state) {
    var callbackUrl = BASE_URL + '/api/auth/soundcloud/callback';
    return 'https://api.soundcloud.com/connect?' + querystring.stringify({
...
function _soundcloudTokenRequest(payload) {
    return new Promise(function (resolve, reject) {
        var postData = JSON.stringify(Object.assign({
            client_id:     SOUNDCLOUD_CLIENT_ID,
        ...
        var req = https.request(options, function (res) {
            ...
            res.on('end', function () {
                ...
                } catch (e) { reject(new Error('SoundCloud token parse error: ' + data.slice(0, 100))); }
            });
        });
        req.on('error', reject);
        req.write(postData);
...
function soundcloudExchangeCode(code) {
    var callbackUrl = BASE_URL + '/api/auth/soundcloud/callback';
    return _soundcloudTokenRequest({
        grant_type:   'authorization_code',
        code:         code,
        redirect_uri: callbackUrl,
    });
...
function soundcloudRefreshToken(refreshToken) {
    return _soundcloudTokenRequest({ grant_type: 'refresh_token', refresh_token: refreshToken })
        .then(function (result) {
            if (!result.refreshToken) result.refreshToken = refreshToken;
            return result;
        });
...
/**
 * Make an authenticated request to the SoundCloud API v2.
 * Handles 301/302 redirects automatically.
 */
function soundcloudRequest(method, path, accessToken) {
    return new Promise(function (resolve, reject) {
        var options = {
            hostname: 'api.soundcloud.com',
            path:     path,
        ...
        var req = https.request(options, function (res) {
            if (res.statusCode === 301 || res.statusCode === 302) {
                ...
                // Strip hostname if present, keep path+query
                var newPath = loc.replace(/^https?:\/\/[^/]+/, '');
                if (!newPath) return reject(new Error('SoundCloud redirect with no location'));
                return soundcloudRequest(method, newPath, accessToken).then(resolve).catch(reject);
            }
            var data = '';
            res.on('data', function (c) { data += c; });
            res.on('end', function () {
                try {
                    ...
                    var json = JSON.parse(data);
                    if (res.statusCode >= 400) {
                        var msg = (json.errors && json.errors[0] && json.errors[0].error_message) ||
                                  'SoundCloud API ' + res.statusCode;
                        return reject(new Error(msg));
                    }
                    resolve(json);
                } catch (e) { reject(new Error('SoundCloud parse error: ' + data.slice(0, 100))); }
            });
        });
        req.on('error', reject);
        req.end();
...
module.exports = {
    ...
    spotifyExchangeCode: spotifyExchangeCode,
    ...
    soundcloudRequest: soundcloudRequest,
};
