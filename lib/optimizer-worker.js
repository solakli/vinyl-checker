...
async function processJob(job) {
    ...
    // Step 1: Wantlist
    progress('wantlist', 0, 0, wantlist.length + ' records on your hit list', {});
    ...
    progress('stores', 0, 0, 'Raiding Gramaphone, Further Records, Octopus…', {});
    var storeInventory = {
    ...
    progress('stores', storeTotal, storeTotal, storeTotal + ' store listings in the crate', {});

    // Step 3: Discogs marketplace listings
    var releaseIds = wantlist.map(function(w) { return w.discogs_id; }).filter(Boolean);
    progress('discogs', 0, releaseIds.length, 'Hitting up the Discogs marketplace…', { listingsFound: 0, sellersFound: 0 });
    ...
    // Count unique sellers across all fetched listings
    ...
    var totalSellers = Object.keys(sellerSet).length;
    progress('optimize', 0, 0,
        totalListings + ' listings from ' + totalSellers + ' sellers — finding best cart…',
