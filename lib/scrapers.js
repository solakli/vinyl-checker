...
function matchInventoryRow(wanted, row) {
    if (wanted && row && wanted.catno && row.catno && catnosMatch(wanted.catno, row.catno)) {
        return true;
    }
...
function recordsMatchCombined(wanted, combinedTitle) {
    ...
    if (norm.indexOf(wantedArtist) !== -1 || artistSim >= 0.7) {
        ...
        if (titleSim >= 0.6 && numbersMatch(wantedTitle, remainder)) return true;
    }
    var fullWanted = normalize(wanted.artist + ' ' + wanted.title);
    if (similarity(fullWanted, norm) >= 0.75 && numbersMatch(fullWanted, norm)) return true;
    // Stores sometimes omit the artist name (e.g. TT Lab, UVS)
    // If the wanted title is exactly contained in the combined title, it's a match
    if (wantedTitle.length >= 6 && norm.indexOf(wantedTitle) !== -1) return true;
    // Also check if the combined title starts with the wanted title (ignoring appended format info)
    // But require catalog numbers to match exactly
    if (wantedTitle.length >= 6 && similarity(wantedTitle, norm.substring(0, wantedTitle.length + 5)) >= 0.85 && numbersMatch(wantedTitle, norm)) return true;
    return false;
}
...
async function checkDeejay(page, item) {
    ...
    try {
        ...
        // Race evaluate against a timeout — Deejay's audio player can hang page.evaluate
        var products = await Promise.race([
        ...
        // Reset page after timeout to prevent cascading failures
...
async function checkItem(workerPages, item) {
    // Catalog-synced stores — pure DB lookup, fire immediately in parallel
    var phonicaPromise = checkPhonica(null, item);
    // Catalog-mirror stores: pure local DB lookups, kicked off in parallel.
    ...
    // Batch 1: 5 browser stores in parallel (one per page)
    ...
    // Batch 2: remaining 2 browser stores (UVS has moved to catalog-mirror)
    var batch2 = await Promise.all([
        checkDecks(workerPages[0], item),
        checkYoyaku(workerPages[4], item)
    ]);

    var results = batch1.concat(batch2);
    results.push(await phonicaPromise);
    results.push(await gramaphonePromise);
    results.push(await uvsPromise);
    results.push(await furtherPromise);
    results.push(await octopusPromise);
    return results;
}
