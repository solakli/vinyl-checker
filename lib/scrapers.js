...
// HHV
async function scrapeHHVProducts(page, searchUrl) {
...
async function checkHHV(page, item) {
    ...
    try {
        ...
        await new Promise(function (r) { setTimeout(r, Math.floor(Math.random() * 1500)); });

        var products = await scrapeHHVProducts(page, searchUrl);

        // Retry once if 0 products (SPA may not have rendered)
        if (products.length === 0) {
            console.log('[HHV] "' + item.searchQuery + '" \u2192 0 products, retrying...');
            await new Promise(function (r) { setTimeout(r, 2000); });
            products = await scrapeHHVProducts(page, searchUrl);
        }
