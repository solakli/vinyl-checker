...
// Extract trailing volume/edition suffix — Roman numerals or bare number at end of title.
// "acid dub versions iii" → "iii", "metro area 3" → "3", "untitled" → null
...
function recordsMatch(wanted, found, threshold) {
    ...
    // Guard: if wantedTitle is purely numeric (e.g. a year like "2025"), the
    // wordContains boost would fire on any title that contains that year.
    // Skip the boost for bare-number titles — require actual string similarity.
    ...
    if (!wantedIsBareNumber && wordContains(wantedTitle, foundTitle)) {
        titleSim = Math.max(titleSim, 0.85);
    }

    // Guard: Roman numeral / volume suffix mismatch.
    // "Acid Dub Versions II" ≠ "Acid Dub Versions III" even though JW similarity
    // is very high (differ by one char). If both titles carry a trailing suffix
    // and those suffixes differ, they are different releases.
...
// page.goto() throws "Requesting main frame too early!" when called while a
// previous navigation is still in flight. This is a Puppeteer race condition
// that happens when a page is reused across items before its internal frame
// state has fully settled. We retry once after a short back-off which clears
// the condition in almost every case.
async function safeGoto(page, url, opts) {
    for (var attempt = 0; attempt < 2; attempt++) {
        try {
            ...
            var msg = e.message || '';
            // Race condition — wait for frame to settle then retry
            if (msg.indexOf('Requesting main frame too early') !== -1) {
                await new Promise(function (r) { setTimeout(r, 600); });
                if (attempt < 1) continue;
            }
            // HTTP/2 protocol error — server rejected the request, no point retrying
            if (msg.indexOf('ERR_HTTP2_PROTOCOL_ERROR') !== -1) {
                throw new Error('http2_rejected: ' + url);
            }
            throw e;
        }
    }
...
function getJunoLink(item) {
    // Juno serves HTTP 451 to the Contabo VPS IP (legal/geo block on the
    // datacenter range) - scraping is impossible from production, so we hand
    // the user a search link instead of silently reporting "not found".
    return { store: 'Juno', inStock: false, matches: [], searchUrl: 'https://www.juno.co.uk/search/?q%5Ball%5D%5B%5D=' + encodeURIComponent(item.searchQuery), linkOnly: true, usShipping: '\u00a37.99' };
}
function getHHVLink(item) {
    var smartQuery = getSmartSearchQuery(item);
    return { store: 'HHV', inStock: false, matches: [], searchUrl: 'https://www.hhv.de/katalog/filter/suche-S11?af=true&term=' + encodeURIComponent(smartQuery), linkOnly: true, usShipping: '\u20ac11.99' };
}
...
async function checkDeejay(page, item) {
    ...
    try {
        await safeGoto(page, searchUrl, { waitUntil: 'domcontentloaded', timeout: 7000 });
        ...
        try {
            ...
            if (evalErr.message === 'evaluate timeout') {
                ...
                await page.goto('about:blank', { timeout: 4000, waitUntil: 'domcontentloaded' }).catch(function () {});
                await new Promise(function (r) { setTimeout(r, 800); });
                // Second attempt — shorter timeout so a stuck retry fails fast
                await safeGoto(page, searchUrl, { waitUntil: 'domcontentloaded', timeout: 7000 });
                await page.waitForSelector('.product h2.artist, .product h3.title', { timeout: 4000 }).catch(function () {});
...
async function checkHHV(page, item) {
    ...
    try {
        ...
        await safeGoto(page, searchUrl, { waitUntil: 'load', timeout: 12000 });
...
async function checkHardwax(page, item) {
    ...
    try {
        await safeGoto(page, searchUrl, { waitUntil: 'domcontentloaded', timeout: 8000 });
...
async function checkYoyaku(page, item) {
    // Yoyaku's WooCommerce search only indexes the release title — querying
    // "Artist Title" returns zero rows even when the record is stocked.
    // Search the title alone; recordsMatch() verifies the artist afterwards.
    var yoyakuQuery = (item.title || item.searchQuery || '').trim() || item.searchQuery;
    var searchUrl = 'https://yoyaku.io/?s=' + encodeURIComponent(yoyakuQuery) + '&post_type=product';
    try {
        await safeGoto(page, searchUrl, { waitUntil: 'domcontentloaded', timeout: 8000 });
        ...
        var products = await page.evaluate(function () {
            ...
            document.querySelectorAll('li.product').forEach(function (el) {
                // WooCommerce stamps stock state directly on the card classes.
                // "Coming soon" pre-orders carry outofstock too, so this also skips those.
                var isOOS = el.classList.contains('outofstock') ||
                            (el.textContent || '').toLowerCase().indexOf('out of stock') !== -1;

                // Current Yoyaku layout (2026): title and artist are separate links --
                //   <a class="woocommerce-loop-product__link">Incidents</a>
                //   <a href="/artist/adsr/">ADSR</a>
                var titleEl = el.querySelector('a.woocommerce-loop-product__link, .woocommerce-LoopProduct-link') ||
                              el.querySelector('.wd-entities-title a, h2 a, .product-title a, .woocommerce-loop-product__title a');
                var combined = titleEl ? titleEl.textContent.trim() : '';

                var artistEl = el.querySelector('a[href*="/artist/"]');
                var artist = artistEl ? artistEl.textContent.trim() : '';
                ...
                var title = combined;
                // Legacy layout fallback: title element held "Artist \u2013 Title" in one string
                if (!artist) {
                ...
                // Keep combined (artist + title when split) so the fallback matcher can use it
                var combo = artist ? (artist + ' - ' + title) : combined;
                items.push({ artist: artist, title: title, combined: combo, price: price });
            });
            return items;
        });
        ...
        console.log('[Yoyaku] "' + yoyakuQuery + '" \u2192 ' + products.length + ' products, ' + matches.length + ' matches');
...
async function checkItem(workerPages, item) {
    ...
    // 4 active browser stores in one parallel batch.
    // HHV (VPS data-centre IP blocked), Turntable Lab (82% error, 0 finds) and
    // Decks.de (86% error, 0 finds) have been demoted to link-only — they resolve
    // immediately and still surface a clickable search URL in the UI.
    // Page assignments: [0]=Deejay  [2]=Juno  [3]=Hardwax  [6]=Yoyaku
    // (indices 1, 4, 5 are reserved / idle so the slot numbering stays stable)
    var browserResults = await Promise.all([
        checkDeejay(workerPages[0], item),             // page 0
        Promise.resolve(getHHVLink(item)),             // link-only (DC IP blocked)
        Promise.resolve(getJunoLink(item)),            // link-only (HTTP 451: VPS IP blocked)
        checkHardwax(workerPages[3], item),            // page 3
        Promise.resolve(getTurntableLabLink(item)),    // link-only (82% error, 0 finds)
        Promise.resolve(getDecksLink(item)),           // link-only (86% error, 0 finds)
        checkYoyaku(workerPages[6], item)              // page 6
    ]);
