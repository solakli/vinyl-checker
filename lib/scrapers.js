...
// Phonica — JSON API via Puppeteer (Sucuri WAF requires JS execution)
async function checkPhonica(page, item) {
    ...
    try {
        // Navigate to API URL — stealth plugin handles Sucuri JS challenge
...
async function checkItem(workerPages, item) {
    ...
    // Batch 3: Decks.de, Phonica, Yoyaku (reuse same 3 pages)
    var batch3 = await Promise.all([
        checkDecks(workerPages[0], item),
        checkPhonica(workerPages[1], item),
        checkYoyaku(workerPages[2], item)
    ]);
    results = results.concat(batch3);
    return results;
}
