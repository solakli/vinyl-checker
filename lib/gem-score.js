 ...
 * > 5%  → 100  (insane engagement, dedicated cult)
 * 2–5%  → 75
 * 1–2%  → 50
 * < 1%  → 25
 * No data → 50
...
function engagementScore(viewCount, likeCount) {
    if (!viewCount || !likeCount) return 50;
    var ratio = likeCount / viewCount;
    if (ratio >= 0.05) return 100;
    if (ratio >= 0.02) return 75;
...
function classifyTier(score, signals) {
    // No YouTube data at all
    if (signals.obscurity === 50 && signals.engagement === 50 && signals.djSignal === 0) {
        if (score >= 55) return 'deep_cut';    // driven by Discogs rarity only
        return 'unscored';
    }

    if (score >= 78) {
        // High obscurity + DJ mentions = hidden gem
        if (signals.djSignal >= 40)  return 'club_weapon';
        return 'hidden_gem';
    }
    if (score >= 55) {
        if (signals.djSignal >= 70)  return 'club_weapon';
        return 'deep_cut';
    }
...
function scoreRelease(meta, rm) {
    ...
    var signals = {
        ...
        engagement: engagementScore(meta.youtube_view_count, meta.youtube_like_count),
    ...
    var tier = classifyTier(gemScore, signals);
...
function scoreUserCollection(userId, db) {
    ...
    var rows = d.prepare(`
        SELECT DISTINCT
            ...
            CASE WHEN w.id IS NOT NULL THEN 1 ELSE 0 END AS in_wantlist,
            CASE WHEN c.id IS NOT NULL THEN 1 ELSE 0 END AS in_collection
        FROM wantlist w
        FULL OUTER JOIN collection c
            ON c.user_id = w.user_id AND c.discogs_id = w.discogs_id
        WHERE COALESCE(w.user_id, c.user_id) = ?
          AND (w.active = 1 OR w.id IS NULL)
          AND COALESCE(w.discogs_id, c.discogs_id) IS NOT NULL
    `).all(userId);
