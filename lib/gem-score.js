...
function classifyTier(score, signals, enriched) {
    // Not enriched — only Discogs rarity is available, YouTube signals are defaults
    if (!enriched) {
        if (score >= 55) return 'deep_cut';   // rarity-driven only
        return 'unscored';
    }

    // Has real YouTube data — always assign a meaningful tier
    if (score >= 78) {
