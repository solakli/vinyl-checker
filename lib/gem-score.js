...
// ─── Scoring weights ──────────────────────────────────────────────────────────
// When shazam_count is present, it contributes 10% drawn from obscurity (-5%) and rarity (-5%).
// Without shazam data the weights revert to original.
const W = {
...
const W_WITH_SHAZAM = {
    obscurity:  0.30,   // slightly reduced — Shazam adds cross-platform obscurity signal
    engagement: 0.25,
    djSignal:   0.20,
    rarity:     0.10,   // slightly reduced — Shazam count also captures demand
    genreDepth: 0.05,
    shazam:     0.10,   // Shazam obscurity (low count = unknown = underground)
};

// ─── Individual scorers (each returns 0–100) ─────────────────────────────────

/**
 * Obscurity: inverse log scale on *annualised* view rate (views ÷ years since upload).
 *
 * Why annualise? A video with 15k views uploaded 15 years ago is far more underground
 * than one with 15k views uploaded 6 months ago. Raw view count ignores age entirely.
 *
 * Annualised rate calibration (same log slope as before, just on views/year):
 *   < 100 /yr    → ~97   (completely buried)
 *   ~1k  /yr     → ~58   (niche, underground)
 *   ~5k  /yr     → ~51   (known in scene)
 *   ~20k /yr     → ~43   (gaining traction)
 *   ~100k/yr     → ~30   (popular)
 *   ~1M  /yr     → ~16   (mainstream)
 *   > 10M/yr     → ~2    (viral/commercial)
 *
 * Fallback: if publishedAt is null, score raw viewCount (legacy behaviour).
 * Min age: 1 month (prevents division by near-zero for brand-new uploads).
 */
function obscurityScore(viewCount, publishedAt) {
    if (viewCount == null) return 50; // no data = neutral
    if (viewCount <= 0)   return 100;

    var effectiveViews = viewCount;

    if (publishedAt) {
        var uploadedMs  = new Date(publishedAt).getTime();
        var ageMs       = Date.now() - uploadedMs;
        var ageYears    = Math.max(ageMs / (365.25 * 24 * 3600 * 1000), 1 / 12); // min 1 month
        effectiveViews  = viewCount / ageYears; // annualised rate
    }

    var log   = Math.log10(Math.max(effectiveViews, 1));
...
/**
 * Shazam obscurity: inverse log scale on Shazam identification count.
 * Low count = fewer people have Shazamed it = more underground.
 * Same curve as YouTube obscurity.
 *
 ...
 * ~1M      → ~25   popular
 * > 10M    → ~5    mainstream
 * null     → 50    no data (neutral — don't penalise unenriched records)
 */
function shazamScore(shazamCount) {
...
function classifyTier(score, signals, enriched) {
    ...
    if (!enriched) {
        if (score >= 42) return 'deep_cut';   // rarity-driven only
        return 'unscored';
    }

    // Club weapon: DJ-validated — DJs are actively playing it
    if (signals.djSignal >= 40) return 'club_weapon';

    // Hidden gem: genuinely obscure (< ~500 views) + above-neutral score
    // Calibrated so a 200-view record with neutral engagement qualifies.
    if (signals.obscurity >= 65 && score >= 38) return 'hidden_gem';

    // Deep cut: underground-leaning (< ~10k views scores ~44+)
    // Threshold set so records with a few thousand views and neutral signals qualify.
    if (score >= 35) return 'deep_cut';

    // Known quantity: enriched but mainstream view count dominates
...
function scoreRelease(meta, rm) {
    ...
    var hasSongstats = meta.songstats_track_id != null;
    var hasShazam    = meta.shazam_count != null || meta.songstats_shazams != null;

    // ── Prefer Songstats signals when available ──
    // shazam_count: use songstats_shazams first (more reliable), fall back to legacy shazam_count
    var shazamCount = meta.songstats_shazams != null ? meta.songstats_shazams : (meta.shazam_count || null);

    // DJ signal: merge comment-parsed DJs with hard Songstats data (Beatport + Traxsource + 1001tracklists)
    var commentDjs = commentData.djs || [];
    var djCommentScore = djScore(commentDjs);

    // Songstats DJ score (harder signal: actual DJ chart appearances)
    var djSongstatsScore = 0;
    if (hasSongstats) {
        var tl    = meta.songstats_tracklist_unique || meta.songstats_tracklist_support || 0;
        var charts = (meta.songstats_beatport_charts || 0) + (meta.songstats_traxsource_charts || 0);
        if (tl >= 20)       djSongstatsScore = 100;
        else if (tl >= 10)  djSongstatsScore = 85;
        else if (tl >= 5)   djSongstatsScore = 70;
        else if (tl >= 2)   djSongstatsScore = 50;
        else if (charts >= 50) djSongstatsScore = 90;
        else if (charts >= 20) djSongstatsScore = 75;
        else if (charts >= 5)  djSongstatsScore = 55;
        else if (charts >= 1)  djSongstatsScore = 35;
    }
    // Take the best of the two DJ signals
    var finalDjScore = Math.max(djCommentScore, djSongstatsScore);

    var signals = {
        obscurity:  obscurityScore(meta.youtube_view_count, meta.youtube_published_at),
        engagement: engagementScore(meta.youtube_view_count, meta.youtube_like_count, meta.youtube_comment_count),
        djSignal:   finalDjScore,
        rarity:     rarityScore(rm && rm.community_have, rm && rm.community_want),
        genreDepth: genreDepthScore(commentData.genres),
        shazam:     shazamScore(shazamCount),
    };

    var weights = hasShazam ? W_WITH_SHAZAM : W;

    var gemScore = Math.round(
        signals.obscurity  * weights.obscurity  +
        signals.engagement * weights.engagement +
        signals.djSignal   * weights.djSignal   +
        signals.rarity     * weights.rarity     +
        signals.genreDepth * weights.genreDepth +
        (hasShazam ? signals.shazam * weights.shazam : 0)
    );
    ...
    return {
        ...
        publishedAt:    meta.youtube_published_at  || null,
        ...
        shazamKey:      meta.shazam_track_key      || null,
        // Songstats signals
        songstatsTrackId:       meta.songstats_track_id          || null,
        songstatsShazams:       meta.songstats_shazams           || null,
        songstatsScStreams:      meta.songstats_sc_streams        || null,
        songstatsSpotifyStreams: meta.songstats_spotify_streams   || null,
        songstatsBeatport:      meta.songstats_beatport_charts   || null,
        songstatsTraxsource:    meta.songstats_traxsource_charts || null,
        songstatsTracklist:     meta.songstats_tracklist_support || null,
        songstatsTracklistUniq: meta.songstats_tracklist_unique  || null,
    };
}
...
function tasteSummary(scoredReleases) {
    ...
    // Underground index: % of releases scoring obscurity >= 50 (~< 3.5k views).
    // Calibrated so genuinely obscure underground vinyl scores as underground.
    // Previous threshold (65 = < 500 views) was too strict — left 98% out.
    var undergroundPct = total > 0
        ? Math.round(scoredReleases.filter(function(r) { return r.signals.obscurity >= 50; }).length / total * 100)
        : 0;
