...
async function fetchSoundCloudComments(trackId, kind) {
    // Primary: Lundehund RapidAPI (/api/track/comments?track_id=)
    // Fallback: SC API v2 with extracted client_id

    const sc4Headers = SC4_HEADERS();
    if (sc4Headers && kind !== 'playlist') {
        try {
            const res = await fetch(
                'https://' + SC4_HOST + '/api/track/comments?track_id=' + trackId + '&limit=200&sort=oldest',
                { headers: sc4Headers }
            );
            if (res.ok) {
                ...
                if (comments.length > 0) {
                    return extractFromComments(comments, 'SoundCloud comments');
                }
                return { artists: [], step: makeStep('SoundCloud comments', 'none', 'No comments on this track') };
            }
        } catch(_) {}
        // fall through to SC API v2 on error
    }
    ...
    try {
        ...
        const res = await fetch(
            'https://api-v2.soundcloud.com/' + resourceType + '/' + trackId +
            '/comments?client_id=' + clientId + '&threaded=0&filter_replies=0&limit=200',
            {
                headers: {
        ...
        const data     = await res.json();
        const comments = Array.isArray(data) ? data : (data.collection || []);
        if (!comments.length) return { artists: [], step: makeStep('SoundCloud comments', 'none', 'No comments on this track') };
        return extractFromComments(comments, 'SoundCloud comments');
    } catch (e) {
...
/**
 * Two-pass extraction from SC comment objects:
 *
 * Pass 1 — "tracklist comment": look for one comment that contains
 *   5+ formatted lines (timestamps / numbered / dashes). Classic format.
 *
 * Pass 2 — "ID answer mining": SC comments often look like:
 *   [5:28] "id?" + "@user: Maurizio verbeni"
 *   [15:21] "@diskush think it's los Diablos locos - soulfuric"
 *   [58:29] "Jaimy & Con Am - Emotions"  (standalone at same ts as id? question)
 * We group by timestamp, detect ID questions, then extract artist names from
 * reply-style comments at the same position in the mix.
 */
function extractFromComments(commentObjs, sourceName) {
    const bodies = commentObjs.map(function(c) { return c.body || ''; });

    // Pass 1: classic tracklist comment
    const classic = parseCommentsForTracklist(bodies, sourceName);
    if (classic.artists.length > 0) return classic;

    // Pass 2: ID-answer mining
    const artists = parseScIdAnswers(commentObjs);
    if (artists.length > 0) {
        return {
            artists: artists,
            step: makeStep(sourceName, 'ok',
                artists.length + ' artists from ID answers in comments')
        };
    }
...
/**
 * Mine SC comment objects for "ID?" + answer pairs.
 *
 * Groups comments by their timestamp (5-second buckets). At positions where
 * someone asks "id?" or "what's this?", sibling comments at the same timestamp
 * are likely answers. Also picks up standalone "Artist – Title" lines and
 * @reply patterns anywhere in the thread.
 */
function parseScIdAnswers(commentObjs) {
    // Detect ID questions
    const ID_Q = /\bid\b[^a-z]|\bid[\s?!]+|what(?:'?s|\s+is)?\s+this|anyone.{0,15}id|track\s*id|song\s*id|id\s*please|id\s*\?/i;

    // Noise patterns — clear non-answers
    const NOISE = /^(?:https?:\/\/|any\s|did\s|still\s|nope|yeah|nah|thx|thanks|cheers|lol|haha|omg|wow|me\s|us\s|same|crazy|sick|fire|love|great|amazing|nice|beautiful|incredible|wait|found\s|looking|manage|nothing|no[\s,!]+|nah\b|@.{1,25}(?:any\s|did\s|still\s|luck|found|looking|manage|nothing))/i;
    const SENTENCE = /\b(?:luck|you've|you\s|it[\u2019']s\s+a\b|this\s+is\b|these\s+are\b|the\s+one\b|anyone|could\s|would\s|should\s|have\s|been\s|with\s+the\b|going\s+to\b|trying\s+to\b|good\s+sir\b|banger\b)\b/i;

    // Group by 5-second timestamp bucket
    const buckets = new Map();
    for (var i = 0; i < commentObjs.length; i++) {
        var c = commentObjs[i];
        var bucket = Math.floor(((c.timestamp) || 0) / 5000) * 5000;
        if (!buckets.has(bucket)) buckets.set(bucket, []);
        buckets.get(bucket).push((c.body || '').trim());
    }
    ...
    function tryExtract(text) {
        ...
        if (SENTENCE.test(text)) return;

        // Strip @mention prefix if present
        var clean = text.replace(/^@\S+:?\s+/, '').trim();
        if (!clean || clean.length < 3) return;

        // "think it's / it's / it is" prefix (handle both smart ' and regular ')
        clean = clean.replace(/^(?:think\s+)?it[\u2019']?s\s+/i, '').trim();
        // "ID:" / "Track ID:" / "Song ID:" prefix
        clean = clean.replace(/^(?:track|song)?\s*id:\s*/i, '').trim();

        if (!clean || clean.length < 3 || NOISE.test(clean)) return;
        // Filter URLs, catalog-number-only strings, trailing emoticons
        if (/^https?:\/\/|youtu\.be|youtube\.com/.test(clean)) return;
        if (/^[A-Z]{2,6}\d{3,}/.test(clean)) return;           // catalog numbers like BSU003
        if (/[:;][)(D]$|<3$|!$/.test(clean)) return;           // trailing emoticon or !

        // Repeated-character expressions: "Yoooo", "siiick", "wauwww" etc.
        if (/(.)\1{2,}/.test(clean)) return;

        // Unicode emoji — surrogate pairs (U+1F000+) and misc symbol blocks
        if (/[\u2600-\u27BF\u2300-\u23FF\uD83C-\uDBFF\uDC00-\uDFFF]/.test(clean)) return;

        // Check for "Artist - Title" split
        var dashIdx = clean.search(/\s+[-–]\s+/);
        if (dashIdx > 0) {
            var artist = clean.slice(0, dashIdx).trim();
            // Artist left-side must: start with capital/digit, be 2-50 chars,
            // contain a letter, and NOT look like a sentence fragment
            // Also reject: digit-start + sentence words e.g. "143 is DJ Senze"
            if (artist.length >= 2 && artist.length <= 50
                && /[a-zA-Z]/.test(artist)
                && /^[A-Z\d]/.test(artist)
                && !/\b(?:the\s+track|this\s+track|that\s+is|what\s+is|more\s+like|around\s+|starting\s+|from\s+the\b)\b/i.test(artist)
                && !(/^\d/.test(artist) && /\b(?:is|are|was|were|the\b|a\b|an\b)\b/i.test(artist))) {
                artists.add(artist);
            }
            return;
        }

        // No dash — proper name: must start with capital, ≤ 4 words,
        // no common articles, no pure-expression patterns
        var words = clean.split(/\s+/);
        if (words.length <= 4 && /^[A-Z]/.test(clean) && clean.length >= 3 && clean.length <= 50) {
            if (!/\b(?:a|the|of|in|is|it|on|at|to|for|and|but|or|so|yet|nor|as)\b/i.test(clean)
                && !/^(?:Yes|No|Ok|Wow|Lol|Omg|Fire|Sick|Nice|Cool|Damn|Haha|Yoo+)\b/i.test(clean)) {
                artists.add(clean);
            }
        }
    }

    // Properly-capitalised standalone "Artist - Title" pattern
    // Both sides must start with a capital — filters out noise like "what a day - mix"
    var STANDALONE_AT = /^([A-Z\d][^-\n]{1,50}?)\s+[-–]\s+([A-Z\d][^-\n]{1,60})$/;

    buckets.forEach(function(bodies) {
        bodies.forEach(function(body) {
            if (!body) return;

            // "Track ID: Artist - Title" or "Song ID: Artist - Title" — answer pattern,
            // must be checked BEFORE the ID_Q skip (track\s*id would otherwise reject it)
            if (/^(?:track|song)\s*id\s*:\s*/i.test(body) && body.includes(' - ')) {
                tryExtract(body);
                return;
            }

            // Skip pure ID questions (no useful answer in them)
            if (ID_Q.test(body)) return;

            // Always try @reply patterns and "ID:" / "think it's" patterns
            if (/^@/.test(body) || /^id:\s*\S/i.test(body) || /(?:think\s+)?it[\u2019']?s\s+/i.test(body)) {
                tryExtract(body);
                return;
            }

            // Standalone "Artist - Title": extract whenever it looks like proper names
            // (both sides start with capital/digit, no @ prefix, not a question)
            if (body.includes(' - ') && !body.includes('?') && STANDALONE_AT.test(body)) {
                tryExtract(body);
            }
        });
    });
    ...
    const seen = new Set();
    return Array.from(artists).filter(function(a) {
        var k = a.toLowerCase();
...
async function acrcloudFingerprint(url, platform, tokens) {
    ...
    //
    // Offsets: if we know the mix duration, pick 6 random points spread across
    // it (skipping the first/last 90s). Otherwise fall back to fixed spread.
    //
    const durationMs = tokens.durationMs || 0;
    const durationSec = Math.floor(durationMs / 1000);
    const NUM_TRIES   = 6;

    function randomOffsets(totalSec, count) {
        var minSec  = 90;
        var maxSec  = Math.max(minSec + 120, totalSec - CONFIRM_GAP - DURATION - 60);
        var offsets = [];
        var tries   = 0;
        while (offsets.length < count && tries++ < count * 5) {
            var sec = Math.floor(Math.random() * (maxSec - minSec) + minSec);
            // Keep at least 90s apart so we don't double-sample the same track
            var tooClose = offsets.some(function(o) { return Math.abs(o - sec) < 90; });
            if (!tooClose) offsets.push(sec);
        }
        return offsets.sort(function(a, b) { return a - b; });
    }

    const OFFSETS = durationSec > 300
        ? randomOffsets(durationSec, NUM_TRIES)
        : [90, 300, 600, 1200, 1800, 2400]; // fallback for unknown/short duration
    ...
    var durStr  = durationSec > 0 ? ' across ' + Math.round(durationSec / 60) + 'min mix' : '';
    return {
        artists:   identified.map(function(t) { return t.artist; }),
        rawTracks: identified,
        step: makeStep('Audio fingerprinting', 'ok',
            identified.length + ' confirmed via ' + apiUsed + durStr +
            (mismatches ? ' · ' + mismatches + ' unconfirmed skipped' : ''))
    };
}

// ─── SoundCloud via Lundehund RapidAPI (soundcloud-api4) ─────────────────────
//
// Primary path for single tracks: /api/track/info?url= and /api/track/comments
// Falls through to SC API v2 (client_id extraction) for playlists + user likes.

const SC4_HOST    = 'soundcloud-api4.p.rapidapi.com';
const SC4_HEADERS = function() {
    var key = process.env.RAPIDAPI_KEY;
    return key ? {
        'x-rapidapi-key':  key,
        'x-rapidapi-host': SC4_HOST,
    } : null;
};

// ─── SoundCloud public API v2 (fallback for playlists + user likes) ──────────
//
// Strategy: SC embeds a client_id in their own JavaScript bundles.
// We fetch it once, cache it for 24h.
...
async function getSoundCloudClientId() {
    ...
    try {
        const pageRes = await fetch('https://soundcloud.com/', {
            headers: { 'User-Agent': SC_HEADERS['User-Agent'] }
        });
...
/**
 * Resolve a SC URL.
 * Primary path: Lundehund /api/track/info?url= (works for single tracks)
 * Fallback: oEmbed + SC API v2 (works for playlists + user profiles too)
 */
async function resolveSoundCloud(url) {
    // Strip /likes, /reposts etc. — resolve want the base resource URL
    const cleanUrl = url.replace(/\/(likes|reposts|sets|tracks|albums|playlists)\/?$/, '');
    const wantsLikes = /\/likes\/?$/.test(url);

    // ── Primary: Lundehund track/info (single tracks only, fast + reliable) ──
    const sc4Headers = SC4_HEADERS();
    if (sc4Headers && !/\/sets\//.test(cleanUrl)) {
        try {
            const r = await fetch(
                'https://' + SC4_HOST + '/api/track/info?url=' + encodeURIComponent(cleanUrl),
                { headers: sc4Headers }
            );
            if (r.ok) {
                const d = await r.json();
                if (d && d.id && d.kind === 'track') {
                    const user = d.user || {};
                    return {
                        meta: {
                            id:          d.id,
                            kind:        'track',
                            title:       d.title || null,
                            artist:      user.username || user.full_name || null,
                            description: d.description || '',
                            artworkUrl:  d.artwork_url || null,
                            durationMs:  d.full_duration || d.duration || null,
                            trackCount:  null,
                            likeCount:   null,
                            wantsLikes:  false,
                        }
        ...
        // fall through for playlists, user profiles, or API errors
    }
    ...
    try {
        ...
        // oEmbed HTML has the SC API URL URL-encoded, e.g.:
        //   url=https%3A%2F%2Fapi.soundcloud.com%2Ftracks%2F809879380
        const idMatch  = (oembed.html || '').match(/%2F(tracks|playlists|users)%2F(\d+)/i)
        ...
        if (clientId && scId) {
            ...
            try {
                const r = await fetch(endpoint + '?client_id=' + clientId, { headers: SC_HEADERS });
...
async function resolveMixUrl(url, tokens) {
    ...
    if (platform === 'mixcloud') {
        ...
        const acr = await acrcloudFingerprint(url, platform,
            Object.assign({}, tokens, { durationMs: mcMeta && mcMeta.durationMs || 0 }));
        steps.push(acr.step);
        return {
            platform,
    ...
    // Step 3: audio fingerprinting — inject duration so we can randomise offsets
    const acrTokens = Object.assign({}, tokens, { durationMs: meta && meta.durationMs || 0 });
    const acr = await acrcloudFingerprint(url, platform, acrTokens);
    steps.push(acr.step);
...
function searchInventoryForTracklist(tracklistArtists, db) {
    ...
    // Primary: pre-indexed store inventory (Further, Gramaphone, Octopus, UVS, Hardwax…)
    ...
    // Secondary: wantlist-based store_results (Deejay.de, HHV when available)
    // Pull all in-stock wantlist matches and treat them as additional inventory rows
    try {
        const srRows = db.prepare(
            'SELECT DISTINCT w.artist, sr.store, sr.matches, sr.search_url ' +
            'FROM store_results sr JOIN wantlist w ON w.id = sr.wantlist_id ' +
            'WHERE sr.in_stock = 1 AND sr.matches != \'[]\' AND sr.matches IS NOT NULL'
        ).all();
        srRows.forEach(function(sr) {
            var matches = [];
            try { matches = JSON.parse(sr.matches || '[]'); } catch(_) {}
            matches.forEach(function(m) {
                rows.push({
                    store:        sr.store,
                    artist:       m.artist || sr.artist || null,
                    title:        m.title  || null,
                    label:        null,
                    catno:        null,
                    price_usd:    null,
                    url:          m.url || sr.search_url || null,
                    image_url:    null,
                    product_type: 'Vinyl'
                });
            });
