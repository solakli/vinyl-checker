...
// RapidAPI SoundCloud scraper host (Pat92 / soundcloud-api3 mirrors the official SC API)
// Override via SC_RAPIDAPI_HOST env var if you subscribe to a different scraper variant.
const SC_RAPIDAPI_HOST = process.env.SC_RAPIDAPI_HOST || 'soundcloud-api3.p.rapidapi.com';

async function fetchSoundCloudComments(trackId, kind) {
    const rapidApiKey = process.env.RAPIDAPI_KEY;
    if (!rapidApiKey) {
        return { artists: [], step: makeStep('SoundCloud comments', 'skip', 'No RAPIDAPI_KEY configured') };
    }
    // SC uses /playlists/{id}/comments for sets, /tracks/{id}/comments for singles
    const resourceType = (kind === 'playlist') ? 'playlists' : 'tracks';
    try {
        const res = await fetch(
            'https://' + SC_RAPIDAPI_HOST + '/' + resourceType + '/' + trackId + '/comments?limit=200',
            {
                headers: {
                    'X-RapidAPI-Key':  rapidApiKey,
                    'X-RapidAPI-Host': SC_RAPIDAPI_HOST,
                }
            }
        );
        if (!res.ok) {
...
async function resolveSoundCloud(url) {
    const rapidApiKey = process.env.RAPIDAPI_KEY;
    if (!rapidApiKey) return { error: 'No RAPIDAPI_KEY configured — add it to .env', meta: null };
    // Strip /likes, /sets, /tracks suffixes — resolve always wants the base resource URL
    const cleanUrl = url.replace(/\/(likes|reposts|sets|tracks|albums|playlists)\/?$/, '');
    try {
        const res = await fetch(
            'https://' + SC_RAPIDAPI_HOST + '/resolve?url=' + encodeURIComponent(cleanUrl),
            {
                headers: {
                    'X-RapidAPI-Key':  rapidApiKey,
                    'X-RapidAPI-Host': SC_RAPIDAPI_HOST,
        ...
        // For user profiles, data IS the user object (no nested .user)
        const user = kind === 'user' ? data : (data.user || {});
        return {
            meta: {
                id:          id,
                kind:        kind,  // 'track', 'playlist', or 'user'
                title:       data.title || null,
                artist:      user.username || user.full_name || null,
                avatarUrl:   kind === 'user' ? (data.avatar_url || null) : null,
                description: data.description || '',
                artworkUrl:  data.artwork_url || (kind === 'user' ? data.avatar_url : null) || null,
                trackCount:  data.track_count || null,
                likeCount:   kind === 'user' ? (data.public_favorites_count || null) : null,
                // Flag when user pasted a /likes URL explicitly
                wantsLikes:  /\/likes\/?$/.test(url),
            }
        };
    } catch (e) { return { error: e.message, meta: null }; }
}

// ─── SoundCloud user likes scraper ───────────────────────────────────────────

/**
 * Extract the best artist name from a SC liked track object.
 * Priority: publisher_metadata.artist > "Artist - Title" title parse > uploader username
 */
...
/**
 * Fetch up to `limit` liked tracks for a SC user ID.
 * Returns { artists, rawTracks, likeCount, step }
 */
async function fetchSoundCloudLikes(userId, limit) {
    limit = limit || 200;
    const rapidApiKey = process.env.RAPIDAPI_KEY;
    if (!rapidApiKey) {
        return { artists: [], rawTracks: [], step: makeStep('SoundCloud likes', 'skip', 'No RAPIDAPI_KEY') };
    }
    try {
        const res = await fetch(
            'https://' + SC_RAPIDAPI_HOST + '/users/' + userId + '/likes?limit=' + limit,
            {
                headers: {
                    'X-RapidAPI-Key':  rapidApiKey,
                    'X-RapidAPI-Host': SC_RAPIDAPI_HOST,
        ...
        if (!res.ok) {
            return { artists: [], rawTracks: [], step: makeStep('SoundCloud likes', 'fail', 'HTTP ' + res.status) };
        }
        const data   = await res.json();
        const tracks = Array.isArray(data) ? data : (data.collection || []);

        if (!tracks.length) {
            return { artists: [], rawTracks: [], step: makeStep('SoundCloud likes', 'none', 'No public likes found') };
        }
        ...
        tracks.forEach(function(track) {
            // Likes can contain playlists too — skip non-tracks
            if (track.kind && track.kind !== 'track') return;
            const artist = artistFromLikedTrack(track);
            if (!artist) return;
            rawTracks.push({ artist, title: track.title || null, id: track.id });
        ...
        return {
            ...
            step: makeStep('SoundCloud likes', 'ok',
                artists.length + ' unique artists from ' + tracks.length + ' liked tracks')
        };
    } catch (e) {
