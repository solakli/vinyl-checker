...
function parseScIdAnswers(commentObjs) {
    ...
    function tryExtract(text) {
        ...
        if (dashIdx > 0) {
            ...
            // Artist side must be 2-60 chars and contain a letter
            if (artist.length >= 2 && artist.length <= 60 && /[a-zA-Z]/.test(artist)) {
                artists.add(artist);
            }
            return;
        }

        // No dash — treat as artist name only if short and looks like a proper name
        // (≤ 4 words, has at least one uppercase letter, no sentence structure)
        var words = clean.split(/\s+/);
        if (words.length <= 4 && /[A-Z]/.test(clean) && clean.length <= 50) {
