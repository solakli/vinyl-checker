...
function parseScIdAnswers(commentObjs) {
    ...
    const NOISE = /^(?:any\s|did\s|still\s|nope|yeah|nah|thx|thanks|lol|haha|omg|wow|me\s|us\s|same|crazy|sick|fire|love|great|amazing|nice|beautiful|incredible|wait|found\s|looking|manage|nothing|no\s+(?:luck|sir|bro|friend|man)|@.{1,25}(?:any\s|did\s|still\s|luck|found|looking|manage|nothing))/i;
    const SENTENCE = /\b(?:luck|you've|you\s|it's\s+a\b|this\s+is\b|these\s+are\b|the\s+one\b|anyone|could\s|would\s|should\s|have\s|been\s|with\s+the\b|going\s+to\b|trying\s+to\b)\b/i;
