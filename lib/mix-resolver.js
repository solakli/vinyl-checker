    ...
    if (sc4Headers && kind !== 'playlist') {
        try {
            const res = await fetch(
                'https://' + SC4_HOST + '/api/track/comments?track_id=' + trackId + '&limit=200',
                { headers: sc4Headers }
            );
            if (res.ok) {
...
function parseScIdAnswers(commentObjs) {
    ...
    buckets.forEach(function(bodies) {
        var hasIdQ = bodies.some(function(b) { return ID_Q.test(b); });

        bodies.forEach(function(body) {
            if (!body || ID_Q.test(body)) return;

            // Always try @reply patterns and "ID:" patterns
            if (/^@/.test(body) || /^id:/i.test(body) || /(?:think\s+)?it'?s\s+/i.test(body)) {
                tryExtract(body);
                return;
            }

            // Standalone "Artist - Title" only extract when near an ID question
            if (hasIdQ && body.includes(' - ')) {
                tryExtract(body);
            }
        });
    });
