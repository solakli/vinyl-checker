...
//
// Replaces the "all 300 items in one Chrome session" approach that OOM-crashed
// the VPS nightly. Instead, each run checks the N items with the oldest
// last_puppeteer_check_at (nulls first), using only 2 live-scraper stores
// (Deejay.de + Juno). The full wantlist cycles through every ~few days depending
// on batch size and run frequency.
//
// Only live-scraper stores run here. Catalog stores (Further, Gramaphone, etc.)
// are handled separately by runCatalogMatch() which needs no Chrome.
//
const ROLLING_BATCH = 50; // items per run per user
...
async function dailyRollingPuppeteer() {
    ...
    try {
        ...
        // We use a single 7-page worker (same as createWorkerPages) but only
        // call checkDeejay (page 0) and checkJuno (page 2). The other pages
        // idle — 7 idle tabs is ~150 MB vs 7 active tabs × 2 workers = OOM.
        var workers = await createWorkerPages(browser, 1);
        var pages = workers[0]; // [deejay, hhv, juno, hardwax, tl, decks, yoyaku]

        var totalChecked = 0, totalInStock = 0, totalChanges = 0;

        for (var u = 0; u < users.length; u++) {
            var user = users[u];
            var items = db.getItemsDueForPuppeteerCheck(user.id, ROLLING_BATCH);
            if (items.length === 0) continue;

            // Snapshot current store_results so we can detect in-stock flips
            var snapshot = db.snapshotStoreResults(user.id);
            var snapshotMap = {};
            snapshot.forEach(function(s) { snapshotMap[s.wantlist_id + ':' + s.store] = s; });

            console.log('[rolling] ' + user.username + ': ' + items.length + ' items (Deejay.de + Juno)');
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                try {
                    ...
                    results.forEach(function(r) {
                        if (r.inStock) totalInStock++;
                        var key = w.id + ':' + r.store;
                        var old = snapshotMap[key];
                        var change = detectChanges(
                            user.id, w.id, r.store,
                            old ? old.in_stock : 0,
                            old ? old.matches  : '[]',
                            r
                        );
                        if (change) totalChanges++;
                    });
                } catch (e) {
                    // Still stamp so this item doesn't get retried every run
                    try { db.stampPuppeteerCheck(w.id); } catch(_) {}
                    totalChecked++;
                }

                // Small gap — spread VPS load, be polite to stores
                await new Promise(function(r) { setTimeout(r, 200); });
            }

            console.log('[rolling] ' + user.username + ': done. in_stock=' + totalInStock + ' changes=' + totalChanges);
        }

        console.log('[rolling] Complete: ' + totalChecked + ' items, ' + totalInStock + ' in stock, ' + totalChanges + ' changes');
    } catch(e) {
