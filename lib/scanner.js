...
const NUM_WORKERS = 5;        // parallel items; was 3 (~56% faster on 274-item wantlists)
...
async function runScan(username, initialSendEvent, force, userDiscogsHeaders) {
    ...
    try {
        ...
        } finally { try { if (browser) await browser.close(); } catch(e) {} browser = null; }
...
async function dailyFullRescan() {
    ...
    for (var u = 0; u < users.length; u++) {
        ...
        try {
            ...
            var itemIdx = 0;
            var changesFound = 0;
            var dailyScanRunId = db.startScanRun(user.id, 'daily');
