...
async function runScan(username, initialSendEvent, force, userDiscogsHeaders) {
    ...
    try {
        ...
        var headers = userDiscogsHeaders
            ? userDiscogsHeaders('GET', '/users/' + username + '/wants?per_page=100&page=1')
            : undefined;
        var wantlist = await discogs.fetchWantlist(username, headers);
