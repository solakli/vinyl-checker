...
const NUM_WORKERS = 2;        // parallel items; 2×7=14 Chrome tabs — balances speed vs VPS RAM with Ronaut Radio
...
async function runScan(username, initialSendEvent, force, userDiscogsHeaders) {
    ...
    try {
        ...
        // Wait for any concurrent Chrome session to finish (max 60 s).
        if (_chromeLock) {
            sendEvent('status', { phase: 'waiting', message: 'Waiting for another scan to free Chrome...' });
            var lockStart = Date.now();
            var lockWaits = 0;
            while (_chromeLock && lockWaits < 12) {
                await new Promise(function (r) { setTimeout(r, 5000); });
                lockWaits++;
            }
            if (_chromeLock) throw new Error('Chrome is busy with another scan — try again in a minute.');
            // Track contention
...
async function backgroundSync() {
    ...
    for (var i = 0; i < users.length; i++) {
        ...
        try {
            ...
            // Background sync uses 1 worker to leave headroom for manual scans / other services
...
async function dailyFullRescan() {
    ...
    for (var u = 0; u < users.length; u++) {
        ...
        try {
            ...
            // Daily rescan: single worker — conserves RAM, leaves room for manual scans between users
            var workers = await createWorkerPages(browser, 1);
            ...
            // Daily rescan: single worker only
            await dailyWorker(0);
            } finally {
