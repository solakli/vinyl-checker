...
function classifyError(msg) {
    ...
    if (msg.indexOf('navigation') !== -1 || msg.indexOf('net::') !== -1 || msg.indexOf('failed to load') !== -1) return 'err_nav';
    if (msg.indexOf('protocol') !== -1 || msg.indexOf('target closed') !== -1 || msg.indexOf('session closed') !== -1) return 'err_protocol';
    if (msg.indexOf('selector') !== -1 || msg.indexOf('queryselector') !== -1 || msg.indexOf('not found') !== -1) return 'err_selector';
    return 'err_other';
}
...
async function validateInStockResults() {
    ...
    try {
        ...
        // Block media/fonts/images to prevent audio players from hanging
        ...
        for (var i = 0; i < inStockRows.length; i++) {
            ...
            try {
                ...
                ss.errors++;
                var errType = classifyError(e.message);
                ss[errType]++;
                summary.errors++;
                results.push({ artist: row.artist, title: row.title, store: row.store, phase: 'FP-check',
        ...
        for (var j = 0; j < notInStockRows.length; j++) {
            ...
            try {
                ...
                ss2.errors++;
                var errType2 = classifyError(e.message);
                ss2[errType2]++;
                summary.errors++;
                results.push({ artist: row2.artist, title: row2.title, store: row2.store, phase: 'FN-check',
