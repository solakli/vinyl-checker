...
const NUM_WORKERS = 3; // 3 items checked simultaneously (was 2)
const STORES_PER_WORKER = 3; // 3 browser pages per worker (reused across store batches)
...
async function createWorkerPages(browser) {
    ...
    for (var w = 0; w < NUM_WORKERS; w++) {
        ...
        // No request interception — stealth plugin requires clean browser state
        // for HHV's bot detection bypass
...
// ═══════════════════════════════════════════════════════════════
// STOCK VALIDATION JOB
// Periodically re-checks items marked "in stock" to verify they're
// actually available. Catches false positives from stale data or
// scrapers missing "out of stock" indicators.
// ═══════════════════════════════════════════════════════════════

var validationStats = {
    lastRun: null,
    totalChecked: 0,
    totalFlipped: 0,  // in_stock → out_of_stock
    totalConfirmed: 0,
    totalErrors: 0,
    runCount: 0,
    lastResults: []  // last batch details
};
...
async function validateInStockResults() {
    ...
    try {
        ...
        // Get all in_stock results that aren't link-only, grouped by store
        ...
        if (inStockRows.length === 0) {
            console.log('[validator] No in-stock results to validate');
            delete activeScans['_validator'];
            return;
        }

        // Validate up to 30 items per run to avoid long browser sessions
        var BATCH_SIZE = 30;
        var toValidate = inStockRows.slice(0, BATCH_SIZE);

        console.log('[validator] Validating ' + toValidate.length + ' / ' + inStockRows.length + ' in-stock results');

        // Launch a single browser with one page per store type
        var browser = await puppeteer.launch({
            headless: 'new',
            args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage',
        ...
        for (var i = 0; i < toValidate.length; i++) {
            var row = toValidate[i];
            var searchQuery = (row.artist + ' ' + row.title).trim();
            var item = { artist: row.artist, title: row.title, searchQuery: searchQuery, catno: row.catno, label: row.label };

            try {
                var result = await validateSingleResult(page, row.store, row.search_url, item);

                if (!result.verdict) {
                    // Flip to out of stock
                    d.prepare('UPDATE store_results SET in_stock = 0, matches = ?, checked_at = ? WHERE id = ?')
                        .run('[]', new Date().toISOString(), row.id);
                    flipped++;
                    var failStep = !result.step1 ? 'NO_RESULTS' : !result.step2 ? 'NO_MATCH' : 'OUT_OF_STOCK';
                    console.log('[validator] FLIPPED: ' + row.artist + ' - ' + row.title + ' @ ' + row.store + ' → ' + failStep + ' (' + result.reason + ')');
                    results.push({ artist: row.artist, title: row.title, store: row.store, status: 'flipped', failStep: failStep, reason: result.reason });
                } else {
                    confirmed++;
                    results.push({ artist: row.artist, title: row.title, store: row.store, status: 'confirmed',
                                   productsFound: result.productsFound, matchCount: result.matchCount, inStockCount: result.inStockCount });
                }
                ...
                errors++;
                console.log('[validator] ERROR checking ' + row.artist + ' - ' + row.title + ' @ ' + row.store + ': ' + e.message);
                results.push({ artist: row.artist, title: row.title, store: row.store, status: 'error', error: e.message });
            }
        }
        ...
        validationStats.totalChecked += toValidate.length;
        validationStats.totalFlipped += flipped;
        validationStats.totalConfirmed += confirmed;
        validationStats.totalErrors += errors;
        validationStats.runCount++;
        validationStats.lastResults = results;

        console.log('[validator] Done: ' + confirmed + ' confirmed, ' + flipped + ' flipped to out-of-stock, ' + errors + ' errors');
...
// Validate a single store result with 3-step verification:
// Step 1: Did the search return any results? (page loads, products found)
// Step 2: Does any result actually match this record? (not just recommendations)
// Step 3: Is the matched item actually in stock? (not sold out / out of stock)
async function validateSingleResult(page, store, searchUrl, item) {
    ...
    // Generic product extractor per store
    var allProducts = [];
    var soldOutText = ['sold out', 'out of stock', 'ausverkauft', 'nicht verfügbar'];

    if (store === 'Deejay.de') {
        await page.waitForSelector('.product h2.artist, .product h3.title', { timeout: 4000 }).catch(function() {});
        allProducts = await page.evaluate(function() {
            var items = [];
            document.querySelectorAll('.product').forEach(function(el) {
                if (el.classList.contains('equip')) return;
                var artist = (el.querySelector('h2.artist') || {}).textContent || '';
                var title = (el.querySelector('h3.title') || {}).textContent || '';
                var text = el.textContent.toLowerCase();
                items.push({ artist: artist.trim(), title: title.trim(), pageText: text });
            });
            return items;
        });
    } else if (store === 'HHV') {
        ...
        allProducts = await page.evaluate(function() {
            ...
            document.querySelectorAll('.items--shared--gallery-entry--base-component:not(.overlay)').forEach(function(el) {
                var artist = (el.querySelector('span.artist') || {}).textContent || '';
                var title = (el.querySelector('span.title') || {}).textContent || '';
                var text = el.textContent.toLowerCase();
                items.push({ artist: artist.trim(), title: title.trim(), pageText: text });
            });
            return items;
        ...
        allProducts = await page.evaluate(function() {
            ...
            document.querySelectorAll('.product, .searchresult').forEach(function(el) {
                var artist = (el.querySelector('.artist, .catlink') || {}).textContent || '';
                var title = (el.querySelector('.title, .linklist a') || {}).textContent || '';
                var text = el.textContent.toLowerCase();
                items.push({ artist: artist.trim(), title: title.trim(), pageText: text });
            });
            return items;
        ...
        allProducts = await page.evaluate(function() {
            ...
            document.querySelectorAll('.product-card, .product_info').forEach(function(el) {
                var artist = (el.querySelector('.product-artist, .product_info_artist') || {}).textContent || '';
                var title = (el.querySelector('.product-title, .product_info_title') || {}).textContent || '';
                var text = el.textContent.toLowerCase();
                items.push({ artist: artist.trim(), title: title.trim(), pageText: text });
            });
            return items;
        ...
        allProducts = await page.evaluate(function() {
            ...
            document.querySelectorAll('.product-card, .grid-item').forEach(function(el) {
                var combined = (el.querySelector('.product-card__title, h2, h3') || {}).textContent || '';
                var text = el.textContent.toLowerCase();
                items.push({ artist: '', title: combined.trim(), pageText: text });
            });
            return items;
        });
        ...
        allProducts = await page.evaluate(function() {
            ...
            document.querySelectorAll('.product-card.product-grid').forEach(function(el) {
                ...
                var text = el.textContent.toLowerCase();
                items.push({ artist: artist, title: title, pageText: text });
            });
            return items;
        });
        ...
        allProducts = await page.evaluate(function() {
            ...
            document.querySelectorAll('.product_container, .product').forEach(function(el) {
                var artist = (el.querySelector('.artist') || {}).textContent || '';
        ...
        // Unknown store
        return { step1: true, step2: true, step3: true, verdict: true };
    }

    // STEP 1: Did the search return any products?
    if (allProducts.length === 0) {
        return { step1: false, step2: false, step3: false, verdict: false,
                 reason: 'No products found on page', productsFound: 0 };
    }

    // STEP 2: Does any product actually match this record?
    var matchingProducts = allProducts.filter(function(p) {
        if (p.artist) return scrapers.recordsMatch(item, p);
        return scrapers.recordsMatchCombined(item, p.title);
    });

    if (matchingProducts.length === 0) {
        return { step1: true, step2: false, step3: false, verdict: false,
                 reason: 'Products found but none match record', productsFound: allProducts.length };
    }

    // STEP 3: Is the matched item actually in stock?
    var inStockMatches = matchingProducts.filter(function(p) {
        return !soldOutText.some(function(t) { return p.pageText.indexOf(t) !== -1; });
    });

    if (inStockMatches.length === 0) {
        return { step1: true, step2: true, step3: false, verdict: false,
                 reason: 'Record found but out of stock', productsFound: allProducts.length, matchCount: matchingProducts.length };
    }

    return { step1: true, step2: true, step3: true, verdict: true,
             productsFound: allProducts.length, matchCount: matchingProducts.length, inStockCount: inStockMatches.length };
}
