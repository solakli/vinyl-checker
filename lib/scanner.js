...
async function dailyRollingPuppeteer() {
    ...
    try {
        ...
        // Create ROLLING_WORKERS concurrent worker sets.
        // Each set = 7 browser pages (one per store). With 12 GB RAM, 2 sets
        // (~300 MB) is comfortable; bump ROLLING_WORKERS=2 on VPS for double throughput.
        var allWorkers = await createWorkerPages(browser, ROLLING_WORKERS);

        // Shared stats across both phases
        var stats = { inStock: 0, changes: 0, errors: 0, urgent: 0, routine: 0 };
        ...
        for (var u = 0; u < users.length; u++) {
            ...
            if (ROLLING_WORKERS >= 2) {
                ...
                var urgentAbort = false;
                for (var i = 0; i < urgent.length && !urgentAbort; i += 2) {
                ...
                if (urgentAbort) console.error('[rolling] ' + user.username + ': browser irrecoverable, aborting urgent phase');
            } else {
                for (var i = 0; i < urgent.length; i++) {
                    ...
                    if (urgentOutcome === 'abort') {
                        console.error('[rolling] ' + user.username + ': browser irrecoverable, aborting urgent phase');
                        break;
                    }
        ...
        for (var u = 0; u < users.length; u++) {
            ...
            if (ROLLING_WORKERS >= 2) {
                ...
                for (var i = 0; i < routine.length && !routineAbort; i += 2) {
                ...
                if (routineAbort) console.error('[rolling] ' + user.username + ': browser irrecoverable, aborting routine phase');
            } else {
                for (var i = 0; i < routine.length; i++) {
                    ...
                    if (routineOutcome === 'abort') {
                        console.error('[rolling] ' + user.username + ': browser irrecoverable, aborting routine phase');
                        break;
                    }
                    await new Promise(function(r) { setTimeout(r, 200); });
                }
