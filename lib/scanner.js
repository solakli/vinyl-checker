    ...
    try {
        ...
        for (var u = 0; u < users.length; u++) {
            ...
            if (ROLLING_WORKERS >= 2) {
                ...
                for (var i = 0; i < urgent.length; i += 2) {
                    var batch = [
                        _rollingCheckItem(allWorkers[0], urgent[i],     user.id, snapshotMap, stats),
                        urgent[i + 1]
                            ? _rollingCheckItem(allWorkers[1], urgent[i + 1], user.id, snapshotMap, stats)
                            : Promise.resolve()
                    ];
                ...
                for (var i = 0; i < urgent.length; i++) {
                    await _rollingCheckItem(allWorkers[0], urgent[i], user.id, snapshotMap, stats);
                    stats.urgent++;
        ...
        for (var u = 0; u < users.length; u++) {
            ...
            if (ROLLING_WORKERS >= 2) {
                for (var i = 0; i < routine.length; i += 2) {
                    var batch = [
                        _rollingCheckItem(allWorkers[0], routine[i],     user.id, snapshotMap, stats),
                        routine[i + 1]
                            ? _rollingCheckItem(allWorkers[1], routine[i + 1], user.id, snapshotMap, stats)
                            : Promise.resolve()
                    ];
                    await Promise.all(batch);
                ...
                for (var i = 0; i < routine.length; i++) {
                    await _rollingCheckItem(allWorkers[0], routine[i], user.id, snapshotMap, stats);
                    stats.routine++;
                    await new Promise(function(r) { setTimeout(r, 200); });
                }
            }
