...
// ── Catalog-safe matching ─────────────────────────────────────────────────────
//
// recordsMatchCombined was designed for pre-filtered search results (5-20 items).
// Its `norm.indexOf(wantedTitle)` shortcut fires on ANY catalog entry that
// contains the wanted title as a substring — causing false positives when
// scanning 6,000+ items.  This function is stricter:
//
//   • Generic artists (Various, VA, Unknown…) → require catalog number
//     agreement; without one, require near-exact title (≥ 0.92 similarity).
//   • Normal artists → recordsMatch at 0.75 threshold (vs default 0.7).
//   • Combined-title fallback → full-string similarity ≥ 0.85 only; no
//     indexOf shortcut.
...
function matchesItem(scrapers, wanted, row) {
    ...
    // 1. Structured match (Shopify title split cleanly into artist + title)
    if (row.artist && row.title) {
        if (generic) {
            // Various/Unknown: catalog number is the only reliable discriminator.
            ...
            // No catno on either side → require near-exact title to avoid
            // matching Viennatone 001 when we want Viennatone 002, etc.
            var titleSim = scrapers.similarity(wantedTitleNorm, scrapers.normalize(row.title));
            return titleSim >= 0.92 && scrapers.numbersMatch(wantedTitleNorm, scrapers.normalize(row.title));
        }
        // Normal artist: tighter threshold than the Puppeteer scrapers
        if (scrapers.recordsMatch(wanted, { artist: row.artist, title: row.title }, 0.75)) {
            return true;
        }
    }

    // 2. Combined-title fallback (no " - " separator in raw Shopify title)
    if (generic) return false; // too risky without catno + no structured match
