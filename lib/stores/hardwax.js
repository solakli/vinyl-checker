...
// Verified against hardwax.com (April 2026). Some genre slugs that 404'd were removed.
// Run `curl -I https://hardwax.com/SLUG/` to test a new slug before adding it.
const GENRE_SECTIONS = [
    ...
    { path: '/funk/',     tags: ['Funk', 'Soul'] },
    { path: '/hip-hop/',  tags: ['Hip-Hop'] },
    { path: '/reggae-dub/', tags: ['Reggae', 'Dub'] },
    { path: '/electronic/', tags: ['Electronic', 'Experimental'] },
];
    ...
    while ((match = articleRe.exec(html)) !== null) {
        ...
        try {
            ...
            products.push({
                ...
                vendor:          '',
                productType:     'Vinyl',
                tags:            tags,        // array — upsertInventoryItem will JSON.stringify it
                priceUsd:        priceUsd,
                currency:        'EUR',
                available:       1,
                url:             productUrl,
        ...
        for (var i = 0; i < sections.length; i++) {
            ...
            try {
                ...
                products.forEach(function (p) {
                    if (allProducts[p.productId]) {
                        var existing = allProducts[p.productId];
                        // existing.tags may be a raw array (not yet JSON.stringified) or a JSON string
                        var existingTags = Array.isArray(existing.tags) ? existing.tags : JSON.parse(existing.tags || '[]');
                        var newTags      = Array.isArray(p.tags) ? p.tags : JSON.parse(p.tags || '[]');
