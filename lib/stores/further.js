...
// product_type on Further is the FORMAT string: "12"", "LP", "Cassette", "CD", etc.
// Further also carries CDs, cassettes, DVDs, and box-sets through the Juno feed.
// We use a POSITIVE vinyl test: include only when the type contains a vinyl
// format word (LP, inch-mark size, or "Vinyl"). Blank type → assume vinyl.
// This is more robust than a blocklist — new non-vinyl types are excluded by default.
//
//   Include: "12"", "LP", "2xLP", "7"", "3x12"", "LP+CD", "Vinyl"
//   Exclude: "CD", "Cassette", "DVD", "CDr", "2xCD", "Magazine", "Box Set+CD+…"
var VINYL_TYPE_RE = /LP|Vinyl|\d"/;

function shouldInclude(rawProduct) {
    var type = (rawProduct.product_type || '').trim();
    if (!type) return true;   // blank product_type → assume vinyl
    return VINYL_TYPE_RE.test(type);
}
