...
/**
 * Run the full store-only cart optimization for a user.
 *
 * @param {string} username
 * @param {object} opts
 * @param {string} opts.buyerCountry
 * @param {number} [opts.maxPriceUsd]
 * @returns {object} optimizer result (cart, covered, total, uncoveredItems, totals)
 */
function optimizeStoreCart(username, opts) {
    ...
    // Get latest in-stock results per wantlist item / store (most recent checked_at)
    var storeResults = db.getLatestInStockResults(user.id);

    // Build source profiles
    var sources = buildStoreSources(wantlistItems, storeResults, opts);
    if (!sources.length) throw new Error('No in-stock items found in any store. Run a scan first.');
