        ...
        rows.forEach(function(row) {
            ...
            matches.forEach(function(m) {
                ...
                if (!cheapest || priceUsd < cheapest.priceUsd) {
                    cheapest = {
                        itemId:    wantlistId,
                        discogsId: item.discogs_id || null,
...
function buildDiscogsSources(wantlistItems, discogsListings, opts) {
    ...
    discogsListings.forEach(function(row) {
        ...
        if (!bySeller[seller]) bySeller[seller] = {
            ...
            numRatings: row.seller_num_ratings,
            // First non-null actual shipping cost captured by extension (buyer-specific)
            actualShippingUsd: null
        };
        // Capture actual shipping cost if available (prefer first non-null value)
        if (bySeller[seller].actualShippingUsd == null && row.shipping_to_usd != null) {
            bySeller[seller].actualShippingUsd = row.shipping_to_usd;
        }
    ...
    Object.keys(bySeller).forEach(function(seller) {
        ...
        // ships_from='XX' means the Chrome extension saw "Unavailable in [your country]"
        // on the Discogs listing and couldn't read the origin country. These listings
        // cannot be ordered by the current user. Skip them unless the extension also
        // captured an actual shipping cost (which would prove shipping IS possible).
        if ((meta.shipsFrom || 'XX') === 'XX' && meta.actualShippingUsd == null) return;

        // Use actual buyer-specific shipping cost captured by extension when available.
        // Fall back to estimate table only when the extension didn't capture it.
        var shippingCostUsd;
        var shippingSource;
        if (meta.actualShippingUsd != null) {
            shippingCostUsd = meta.actualShippingUsd;
            shippingSource  = 'actual';
        } else {
            shippingCostUsd = shippingLib.estimateShipping(meta.shipsFrom, buyerCountry);
            shippingSource  = 'estimate';
        }
        ...
        sources.push({
            ...
            country: meta.shipsFrom || 'XX',
            sellerRating: meta.rating || null,
            sellerNumRatings: meta.numRatings || null,
            shippingCostUsd: shippingCostUsd,
            shippingSource: shippingSource,   // 'actual' | 'estimate' (for UI / debug)
