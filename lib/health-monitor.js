...
var state = {
    ...
    checks: {
        ...
        rollingScan:    { status: 'unknown', lastOk: null, detail: null },
...
function checkRollingScan(scanner) {
    try {
        ...
        if (!health) { ok('rollingScan', 'no health data yet'); return; }

        var lastRolling = health.lastDailyRescan; // scanner reuses this key for rolling too
        if (!lastRolling) {
            ok('rollingScan', 'Not yet run (startup)');
            return;
        }
        ...
        if (msAgo > 5 * 3600000) {
            warn('rollingScan', 'Rolling scan last ran ' + hAgo + ' h ago — expected every 3 h');
        } else {
            ok('rollingScan', 'Last run ' + hAgo + ' h ago');
        }
    } catch (e) {
        warn('rollingScan', 'Check error: ' + e.message);
    }
}
...
function checkYtQuota() {
    try {
        var d = db.getDb();
        // Look for yt_enrich or yt_id_search jobs that failed with quota errors in last 24h
        var quotaFails = d.prepare(`
            SELECT COUNT(*) as c FROM pipeline_jobs
            WHERE stage IN ('yt_enrich', 'yt_id_search')
              AND status = 'failed'
              AND error LIKE '%quota%'
              AND completed_at > datetime('now', '-24 hours')
        `).get();

        if (quotaFails && quotaFails.c > 0) {
            warn('ytQuota', quotaFails.c + ' quota failure(s) in last 24 h — YouTube enrichment rate-limited');
        } else {
            ok('ytQuota', 'No quota failures in last 24 h');
        }
    } catch (e) {
        ok('ytQuota', 'Check error: ' + e.message);
    }
}
