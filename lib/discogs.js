...
const DISCOGS_TOKEN = process.env.DISCOGS_TOKEN || process.env.DISCOGS_PERSONAL_TOKEN || '';
...
async function fetchCollection(username, userHeadersFn) {
    return new Promise(function (resolve, reject) {
        var fetchPage = function (page, allItems) {
            allItems = allItems || [];
            var pagePath = '/users/' + username + '/collection/folders/0/releases?sort=added&sort_order=desc&per_page=100&page=' + page;
            var headers = (typeof userHeadersFn === 'function')
                ? userHeadersFn('GET', pagePath)
                : (userHeadersFn || discogsHeaders());
            https.get({
                ...
                if (res.statusCode === 404) return reject(new Error('Username not found on Discogs'));
                if (res.statusCode === 403) return reject(new Error('Collection is private'));
                if (res.statusCode === 429) return reject(new Error('Discogs rate limit reached'));
                if (res.statusCode < 200 || res.statusCode >= 300) return reject(new Error('Discogs API error: ' + res.statusCode));
                var data = '';
                res.on('data', function (chunk) { data += chunk; });
                res.on('end', function () {
                    try { var json = JSON.parse(data); } catch (e) { return reject(new Error('Parse error')); }
                    if (!json.releases || json.releases.length === 0) return resolve(allItems);
                    var items = json.releases.map(function (r) {
                        var bi = r.basic_information;
                        var artists = bi.artists || [];
                        var artist = artists.length ? artists[0].name.replace(/\s*\(\d+\)$/, '').trim() : 'Unknown';
                        var labels = bi.labels || [];
                        var label  = labels.length ? labels[0].name : '';
                        var catno  = labels.length ? (labels[0].catno || '') : '';
                        var formats = (bi.formats || []).map(function(f) {
                            var desc = (f.descriptions || []).join(', ');
                    ...
                    if (page < json.pagination.pages) setTimeout(function () { fetchPage(page + 1, allItems); }, 500);
