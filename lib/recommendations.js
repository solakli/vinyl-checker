...
/**
 * Limit results to maxPerStore per store so one catalog store (e.g. Further)
 * can't flood the entire Discovered For You section.
 * Excess items are moved to the end, not discarded.
 */
function diversifyByStore(scored, maxPerStore) {
    maxPerStore = maxPerStore || 8;
    var counts   = {};
    var primary  = [];
    var overflow = [];

    scored.forEach(function (r) {
        var store = (r.item.store || '').toLowerCase();
        counts[store] = (counts[store] || 0) + 1;
        if (counts[store] <= maxPerStore) {
            primary.push(r);
        } else {
            overflow.push(r);
        }
...
function getRecommendations(username, limit) {
    ...
    // Diversity: max 3 items per artist, then max 8 per store
    // — prevents one catalog store flooding Discovered For You
    scored = diversify(scored, 3);
    scored = diversifyByStore(scored, 8);
