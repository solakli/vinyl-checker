...
function compute(userId, username, d) {
    ...
    var candidates = {};  // discogs_id → { release fields, diggersWanting: Set }

    otherRows.forEach(function(row) {
        if (myIds.has(row.discogs_id)) return;  // skip already-owned/wanted
    ...
    var results = Object.values(candidates).map(function(c) {
        ...
        var gemScore     = rawGemScore !== null ? rawGemScore : 50; // neutral default
        var combinedScore = tasteScore * (1 + gemScore / 100);
        ...
        return {
            ...
            combinedScore: Math.round(combinedScore * 100),  // comparable ranking int
            diggersWanting: Array.from(c.diggersWanting),
        };
    });
