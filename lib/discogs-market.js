...
async function fetchListingsForReleases(releaseIds, opts) {
    ...
    releaseIds.forEach(function (id) {
        if (!opts.forceRefresh) {
            ...
            if (cacheAge && new Date(cacheAge.expires_at) > new Date()) {
                ...
                onProgress({ done: done, total: total, releaseId: id, fromCache: true, listingsForRelease: results[id].length });
                return;
            }
        }
        toFetch.push(id);
    ...
    for (var i = 0; i < toFetch.length; i += BATCH_SIZE) {
        ...
        batch.forEach(function (id) {
            ...
            onProgress({ done: done, total: total, releaseId: id, listingsForRelease: results[id].length });
        });
    }
