...
function buildSourcePool(wantlistItems, storeInventory, marketListings, opts) {
    ...
    // Pinned sellers bypass rating filter and are always included
    var pinnedSet = {};
    (opts.pinnedSellers || []).forEach(function(u) { pinnedSet[u.toLowerCase()] = true; });
    ...
    wantlistItems.forEach(function (wanted) {
        ...
        releaseListings.forEach(function (l) {
            ...
            var isPinnedSeller = pinnedSet[l.sellerUsername.toLowerCase()];
            // Pinned sellers bypass the rating filter
            if (!isPinnedSeller && minRating && l.sellerRating && l.sellerRating < minRating) return;
 ...
 */
function optimizeCart(sources, wantlistItems, opts) {
    opts = opts || {};
    var minStoreItems  = parseInt(opts.minStoreItems  || 1, 10);
    var minSellerItems = parseInt(opts.minSellerItems || 1, 10);
    // tasteScores: { sellerUsername → number } — higher means better catalogue fit.
    // Used only as a fractional tiebreaker so it never overrides real cost differences.
    var tasteScores    = opts.tasteScores || {};
    ...
    while (uncovered.size > 0 && iterations++ < maxIterations) {
        ...
        sources.forEach(function (source) {
            ...
            // Taste score: subtract a tiny fraction so sellers whose catalogue matches
            // the user's taste win ties without distorting real cost differences.
            // A taste score of 100 = $0.01 / item advantage — negligible vs real prices.
            var sellerName = source.sellerUsername || '';
            var tasteBonus = (tasteScores[sellerName] || 0) * 0.0001;
            var adjustedScore = score - tasteBonus;
    ...
    // Apply minimum-items-per-order filter (separate threshold for stores vs sellers)
    if (minStoreItems > 1 || minSellerItems > 1) {
        cart = cart.filter(function (c) {
            var isStore = c.source.sourceType === 'store';
            var min = isStore ? minStoreItems : minSellerItems;
            return c.assignedListings.length >= min;
        });
    }
 ...
 *
 * @param {object[]} wantlistItems
 * @param {object}   storeInventory   - { storeKey: rows[] }  catalog-synced stores
 * @param {object}   marketListings   - { discogsId: listings[] } from discogs-market
 * @param {object}   opts
 * @param {object[]} [extraSources]   - additional source profiles (scan stores, Chrome ext listings)
 */
function runOptimizer(wantlistItems, storeInventory, marketListings, opts, extraSources) {
    ...
    return optimizeCart(sources, wantlistItems, opts);
}
